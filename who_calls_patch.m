function [IndVocStartRaw_merge_local, IndVocStopRaw_merge_local, IndVocStartPiezo_merge_local, IndVocStopPiezo_merge_local] = who_calls_patch(Raw_dir, Loggers_dir, Date, ExpStartTime, MergeThresh, Manual,UseOld,CheckMicChannel, varargin)
...
if ~isfile(DataFile)
    ...
    load(fullfile(Raw_dir, sprintf('%s_%s_VocExtractTimes.mat', Date, ExpStartTime)), 'MeanStdAmpRawExtract','Voc_filename','ExtractedSoundDetectionNew','ExtractedSoundDetection')
    ...
    for vv=1:Nvoc
        ...
        ProcessedInd = find(ExtractedSoundDetection==ExtractedSoundDetectionNew(vv));
        if any(ExtractedSoundDetection==ExtractedSoundDetectionNew(vv)) % This call was already extracted through who calls
            %recovering previous results
            IndVocStartRaw_merged(vv) = OLD.IndVocStartRaw_merged(ProcessedInd);
...
%             IndNoiseStopPiezo(vv) = OLD.IndNoiseStopPiezo(ProcessedInd);
            ...
            ManCall = input('Did you hear vocalizations? yes (1) No (0)');
