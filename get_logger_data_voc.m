...
if Nvoc==0
    ...
    fprintf('****** Extracting logger data for %d vocalizations. ******\n', Nvoc);
...
%             play(Player)
            ...
            if sum(isnan(RMS_audioLog))==length(AudioLogs)
                ...
                if ~isempty(HRMS_Ind) && ReAllignment
                    ...
                    Voc_transc_time_refined(vv,2) = VocExt.Voc_transc_time(vv,2); % Keep the same estimate of VocExt.Voc_transc_time(vv,2) in ms
                    clf(F1)
                end
    ...
    if ((OP.bytes/10^9)>=2) || ((OW.bytes/10^9)>=2)
        ...
        save(fullfile(Loggers_dir, sprintf('%s_%s_VocExtractData.mat', Date, ExpStartTime)), 'Piezo_wave', 'Piezo_FS',  'Raw_wave','FS', 'RatioRMS', 'DiffRMS','BandPassFilter', 'AudioLogs', 'RMSHigh', 'RMSLow','VocFilename','Voc_transc_time_refined','LoggerType', 'ReAllignment');
    end
end
